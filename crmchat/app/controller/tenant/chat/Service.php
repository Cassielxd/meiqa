...
class Service extends AuthController
{
     ...
     */
    public function __construct(ChatServiceServices $services,TenantServices $services1)
    {
        parent::__construct();
        $this->services = $services;
        $this->tenantservices = $services1;
    }
    ...
    public function add()
    {
        return $this->success($this->services->createKefuForTent());
    }
    ...
    public function chat_list(ChatServiceDialogueRecordServices $services)
    {
        ...
        $data = CacheService::get('tenant_chat_list' . $this->tenantId);
        if ($data['id']) {
            $where = [

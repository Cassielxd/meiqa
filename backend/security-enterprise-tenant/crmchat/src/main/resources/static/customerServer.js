...
function initCustomerServer(option) {
    ...
    this.settingObj.appid = option.appid || ''; // 租户 appid (游客模式必需)
...
initCustomerServer.prototype.loadIcon = function () {
    ...
    request(iconUrl, 'get', null, this.settingObj.token).then(res => {
        this.settingObj.pcIcon = res.data.icon || this.settingObj.pcIcon;
        this.settingObj.mobileIcon = res.data.icon || this.settingObj.mobileIcon;
        this.runInit();
    }).catch(err => {
        this.runInit();
    })
